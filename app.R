  ...
  sidebarLayout (
    sidebarPanel(
      ...
      numericInput(inputId = "height", label = 
                     "Enter your height (metre,m)", value = 1.2, min = 1, max=500, step=2),
      selectInput(
        inputId = 'item_name',
        label = 'Choose an item from the selected food restaurants ',
        choices = food,
        selected= "Kamikaze Brownie" ),
      
      radioButtons('activity',
                   label=h4('Best Exercise For You (Choose One).'),
    ...
    mainPanel(
      ...
      h3('Chosen Food Item:'),
      h4('Amount of calories been consumed.'),
      DT::dataTableOutput(outputId = "summary"),
      h3('Chosen Activity:'),
      verbatimTextOutput('myactivity'),
      h4('Desired Duration in Minutes:'),    
      verbatimTextOutput('mymins'),    
      h4('Estimated Calories Burned:'),
